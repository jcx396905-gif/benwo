  ...
  Widget _buildDayViewWithSwipe() {
    return PageView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _DayViewContent(date: date, showCompleted: _showCompletedTodos);
      },
    );
  }
...
class _DayViewContent extends ConsumerWidget {
  ...
  const _DayViewContent({required this.date, required this.showCompleted});
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final dayTodos = todos
            .where((t) => _isSameDay(t.scheduledDate, day))
            .toList();
        ...
        return Column(
          children: [
            ...
            DragTarget<TodoItemModel>(
              onWillAcceptWithDetails: (details) =>
                  !_isSameDay(details.data.scheduledDate, day),
              onAcceptWithDetails: (details) {
...
class _MonthViewContentState extends ConsumerState<_MonthViewContent> {
  ...
  Widget _buildCalendarGridContent(
    ...
    for (var day = 1; day <= daysInMonth; day++) {
      ...
      final dayTodos = todos
          .where((t) => _isSameDay(t.scheduledDate, date))
          .toList();
      final filteredTodos = widget.showCompleted
          ? dayTodos
          : dayTodos.where((t) => !t.isCompleted).toList();
      ...
      cells.add(
        DragTarget<TodoItemModel>(
          onWillAcceptWithDetails: (details) =>
              !_isSameDay(details.data.scheduledDate, date),
          onAcceptWithDetails: (details) {
          ...
          builder: (context, candidateData, rejectedData) {
            ...
            return GestureDetector(
              onTap: () {
                setState(() {
                  _selectedDate =
                      _isSameDay(date, _selectedDate ?? DateTime(2000))
                      ? null
                      : date;
                });
              },
    ...
    if (_selectedDate != null) {
      final selectedDayTodos = todos
          .where((t) => _isSameDay(t.scheduledDate, _selectedDate!))
          .toList();
      final filteredTodos = widget.showCompleted
          ? selectedDayTodos
