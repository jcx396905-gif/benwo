...
typedef PomodoroDayParams = ({int userId, DateTime date});
...
final todayPomodoroPlanProvider =
    FutureProvider.family<PomodoroPlanModel, PomodoroDayParams>((ref, params) {
      return ref
          .watch(pomodoroPlanningServiceProvider)
          .ensurePlanAndImportTodos(userId: params.userId, date: params.date);
    });
...
final savedPomodoroListsProvider =
    StreamProvider.family<List<SavedPomodoroListModel>, int>((ref, userId) {
      return ref
          .watch(pomodoroRepositoryProvider)
          .watchSavedPomodoroLists(userId);
    });
...
class PomodoroActions {
  ...
  Future<void> removeTask(PomodoroTaskModel task) {
    return _pomodoroRepository.deleteTask(task.id);
  }
  ...
  Future<int> addSavedListToPlan({
    ...
    if (replaceToday) {
      final completedTaskIds = sessions
          ...
          .toSet();
      final removableIds = existing
          .where(
            (task) => !task.isCompleted && !completedTaskIds.contains(task.id),
          )
          .map((task) => task.id)
          .toList();
      await _pomodoroRepository.deleteTasksByIds(removableIds);
    }

    final baseTasks = replaceToday
        ? await _pomodoroRepository.getTasksByPlanId(plan.id)
        : existing;
    ...
    for (final item in decoded) {
      ...
      final template = _SavedPomodoroTask.fromJson(
  ...
  Future<PomodoroSessionModel> startTask(PomodoroTaskModel task) async {
    await _pomodoroRepository.abandonOtherRunningSessions(userId: task.userId);
  ...
  Future<void> pauseSession(PomodoroSessionModel session) async {
    ...
    session.pausedRemainingSeconds = remaining < 0 ? 0 : remaining;
  ...
  Future<void> completeFocus(PomodoroSessionModel session) async {
    ...
    final elapsedSeconds = _elapsedSeconds(
      ...
      fallbackMinutes: session.focusMinutes,
      completedAt: now,
    );
    session
  ...
  Future<void> completeBreak(PomodoroSessionModel session) async {
    ...
    final now = DateTime.now();
    session
      ..actualBreakSeconds = _elapsedSeconds(
        startedAt: session.breakStartedAt,
        fallbackMinutes: session.breakMinutes,
        completedAt: now,
      )
      ..breakEndAt = now
      ..completedAt = now
  ...
  int _elapsedSeconds({
    ...
    if (startedAt == null) return fallbackMinutes * 60;
  ...
  Future<void> _scheduleSessionEndNotification(
    ...
    if (!await _pomodoroNotificationsEnabled(session.userId)) return;
  ...
  Future<bool> _pomodoroNotificationsEnabled(int userId) async {
    final settings = await _settingsRepository.getSettingsByUserId(userId);
    return settings?.pomodoroNotificationsEnabled ?? true;
  }
